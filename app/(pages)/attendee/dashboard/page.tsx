...
export default async function AttendeeDashboard() {
    ...
    return (
        <div className="min-h-screen bg-[#F8F9FA] font-sans pt-16">
            ...
            <div className="bg-white border-b border-gray-100">
                <div className="mx-auto max-w-7xl px-6 py-8">
                    <div className="flex flex-wrap items-center justify-between gap-4">
                        <div>
                            <p className="text-[11px] font-semibold tracking-widest uppercase text-muted-teal mb-1">
                                My Account
                            </p>
                            <h1 className="text-2xl font-bold text-charcoal-blue">Dashboard</h1>
                        </div>
                        <Link
                            href="/events"
                            className="group inline-flex items-center gap-2 bg-charcoal-blue px-5 py-2.5 text-sm font-semibold text-white transition-colors hover:bg-muted-teal"
                        >
                            Browse Events
            ...
            <div className="mx-auto max-w-7xl px-6 py-8 space-y-6">
                ...
                {/* STATS + NEXT EVENT HERO ROW */}
                <div className="grid grid-cols-1 lg:grid-cols-4 gap-4">

                    {/* NEXT EVENT — spans 2 cols, full-width feel */}
                    <div className="lg:col-span-2 bg-charcoal-blue text-white relative overflow-hidden">
                        {/* Subtle background geometry */}
                        <div className="absolute inset-0 opacity-[0.04]" style={{
                            backgroundImage: `repeating-linear-gradient(45deg, white 0, white 1px, transparent 0, transparent 50%)`,
                            backgroundSize: '20px 20px'
                        }} />
                        <div className="relative z-10 p-7 h-full flex flex-col justify-between min-h-40">
                            {nextEvent && nextEventDate ? (
                                <>
                                    <div className="flex items-start justify-between gap-4">
                                        <div>
                                            ...
                                            <h2 className="text-lg font-bold leading-snug text-white line-clamp-2 max-w-xs">
                                                {nextEvent.event.title}
                                            </h2>
                                        </div>
                                        {/* Countdown block */}
                                        <div className="shrink-0 text-right">
                                            <p className="text-4xl font-black text-white leading-none">{daysToGo}</p>
                                            <p className="text-[10px] font-semibold tracking-widest uppercase text-white/40 mt-1">
                                                days to go
                                            </p>
                                        </div>
                                    </div>
                                    <div className="flex items-end justify-between mt-5 pt-5 border-t border-white/10">
                                        <div className="flex items-center gap-4 text-[12px] text-white/60">
                                            <span className="flex items-center gap-1.5">
                                                ...
                                                {nextEventDate.toLocaleDateString('en-US', { month: 'short', day: 'numeric', year: 'numeric' })}
                                            </span>
                                            <span className="flex items-center gap-1.5">
                                                <svg className="h-3.5 w-3.5 text-muted-teal" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                                ...
                                                {nextEvent.event.location?.split('|')[0] || 'TBD'}
                                            </span>
                                        </div>
                                        <Link
                                            href={`/attendee/ticket/${nextEvent.id}`}
                                            className="inline-flex items-center gap-1.5 bg-muted-teal px-4 py-2 text-[12px] font-semibold text-white transition-colors hover:bg-white hover:text-muted-teal"
                                        >
                                            View Ticket
                                ...
                                <div className="flex flex-col items-start justify-center h-full gap-3">
                                    <p className="text-[10px] font-bold tracking-widest uppercase text-white/40">Next Up</p>
                                    <p className="text-white/60 text-sm">No upcoming events registered.</p>
                                    <Link href="/events" className="text-[12px] font-semibold text-muted-teal hover:underline underline-offset-2">
                                        Browse events →
                                    </Link>
                                </div>
                            )}
                    ...
                    {/* STAT: TOTAL */}
                    <div className="bg-white border border-gray-100 p-6 flex flex-col justify-between">
                        <p className="text-[11px] font-semibold tracking-widest uppercase text-steel-gray">Total Tickets</p>
                        <div>
                            <p className="text-5xl font-black text-charcoal-blue mt-2">{total}</p>
                            <div className="mt-3 h-px w-full bg-gray-100" />
                            <p className="mt-3 text-[12px] text-steel-gray">All registrations</p>
                        </div>
                    </div>

                    {/* STAT: UPCOMING + PAST stacked */}
                    <div className="flex flex-col gap-4">
                        <div className="bg-white border border-gray-100 p-5 flex-1 relative overflow-hidden">
                            <div className="absolute bottom-0 left-0 right-0 h-0.5 bg-muted-teal" />
                            <p className="text-[11px] font-semibold tracking-widest uppercase text-muted-teal">Upcoming</p>
                            <p className="text-4xl font-black text-charcoal-blue mt-1">{activeCount}</p>
                        </div>
                        <div className="bg-white border border-gray-100 p-5 flex-1 relative overflow-hidden">
                            <div className="absolute bottom-0 left-0 right-0 h-0.5 bg-gray-300" />
                            <p className="text-[11px] font-semibold tracking-widest uppercase text-steel-gray">Attended</p>
                            <p className="text-4xl font-black text-charcoal-blue mt-1">{checkedInCount}</p>
                        </div>
                    </div>
                    ...
                    <div className="lg:col-span-2">
                        <div className="bg-white border border-gray-100">
                            ...
                            <div className="flex items-center justify-between px-6 py-4 border-b border-gray-50">
                                ...
                                <span className="text-[11px] font-medium text-steel-gray bg-gray-100 px-2.5 py-1">
                                    {total} total
                                </span>
                            </div>
                                ...
                                <div className="divide-y divide-gray-50">
                                    {tickets.map((ticket) => {
                                        ...
                                        return (
                                            <div
                                                key={ticket.id}
                                                className="group flex items-stretch hover:bg-gray-50/70 transition-colors"
                                            >
                                                {/* Status bar */}
                                                <div className={`w-0.5 shrink-0 ${isActive ? 'bg-muted-teal' : 'bg-gray-150'}`} />
                            ...
                            {/* Footer stats bar */}
                            {tickets.length > 0 && (
                                <div className="flex items-center gap-6 px-6 py-3.5 border-t border-gray-50 bg-gray-50/50">
                        ...
                        <div className="sticky top-24 space-y-4">
                            ...
                            <div className="bg-white border border-gray-100">
                                <div className="flex items-center justify-between px-5 py-3.5 border-b border-gray-50">
                                    <p className="text-[11px] font-bold tracking-widest uppercase text-steel-gray">Recommended</p>
                                    <svg className="h-3.5 w-3.5 text-muted-teal" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                            ...
                            {/* INVITE PROMO */}
                            <div className="bg-charcoal-blue px-5 py-6 text-white relative overflow-hidden">
                                {/* Subtle dot pattern */}
                                <div className="absolute inset-0 opacity-[0.04]" style={{
